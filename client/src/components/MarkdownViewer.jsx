    ...
    return (
        <div class="markdown-viewer">
            <div class="markdown-viewer__actions">
                {user && file && (
                    <button class="markdown-viewer__action-btn" onClick={() => onEdit && onEdit(file.path)} aria-label="문서 수정" title="문서 수정">
                        <IconEdit size={18} />
                    </button>
                )}
                {file && (
                    <button class="markdown-viewer__action-btn" onClick={onDownload} aria-label="파일 다운로드" title="파일 다운로드">
